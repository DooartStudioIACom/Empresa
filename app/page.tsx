  ...
  return <div>
    ...
    {loading ? <div className="mt-10 flex items-center justify-center gap-2 text-xs text-[#718078]"><LoaderCircle className="size-4 animate-spin" /> Carregando rodadas...</div> : !selectedRound ? <section className="mt-6 rounded-2xl border border-dashed border-[#cfdcd4] bg-white px-6 py-14 text-center"><ListChecks className="mx-auto size-7 text-[#668274]" /><h3 className="mt-4 text-sm font-semibold">Nenhuma rodada cadastrada</h3><p className="mt-2 text-xs text-[#78867e]">Crie a primeira rodada com os itens que chegaram da gerência.</p><Button type="button" onClick={() => setNewRoundOpen(true)} className="mt-5 bg-[#173e2c] text-white"><Plus /> Criar rodada</Button></section> : <div className="mt-6 grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
      <div className="space-y-5">
        ...
        <section className="rounded-[22px] border border-[#dce5df] bg-white p-5 sm:p-6">
          <div className="flex items-center justify-between gap-4 border-b border-[#edf1ef] pb-5"><div><p className="detail-label">Plano de testes</p><h3 className="mt-1.5 text-base font-semibold">Itens desta rodada</h3><p className="mt-1 text-xs text-[#7b8981]">Abra cada cartão para registrar o andamento e o resultado.</p></div><Badge className="bg-[#edf5f0] text-[#397657]">{selectedRound.items.length} ITENS</Badge></div>
          <div className="mt-4 grid gap-3">{selectedRound.items.map((item) => { const done = ['Aprovado', 'Com bug'].includes(item.status); return <button type="button" onClick={() => setSelectedItem(item)} key={item.id} className="group grid w-full gap-3 rounded-2xl border border-[#e1e9e4] bg-[#fbfcfb] p-4 text-left transition hover:-translate-y-0.5 hover:border-[#b9ccbf] hover:bg-white hover:shadow-[0_12px_30px_-24px_#173e2c] sm:grid-cols-[42px_1fr_auto]"><span className={`grid size-10 place-items-center rounded-xl ${item.status === 'Aprovado' ? 'bg-[#e9f5ed] text-[#367a52]' : item.status === 'Com bug' ? 'bg-[#fff0ed] text-[#a94a3c]' : item.status === 'Em teste' ? 'bg-[#eaf2fb] text-[#3f72a2]' : 'bg-[#f0f3f1] text-[#89958e]'}`}>{done ? item.status === 'Aprovado' ? <CheckCircle2 className="size-5" /> : <Bug className="size-4" /> : <span className="text-xs font-semibold">{String(item.position).padStart(2, '0')}</span>}</span><span className="min-w-0"><span className="block text-[13px] font-semibold text-[#263a2e]">{item.title}</span><span className="mt-1 block truncate text-[10px] text-[#7d8b83]">{item.path || `Item ${item.position} de ${selectedRound.items.length}`}</span>{item.result_note && <span className="mt-2 line-clamp-2 block text-[10px] leading-4 text-[#65756c]">{item.result_note}</span>}</span><span className="flex items-center gap-2 self-center"><span className={`rounded-full px-2.5 py-1 text-[9px] font-semibold ${item.status === 'Aprovado' ? 'bg-[#e9f5ed] text-[#3e8058]' : item.status === 'Com bug' ? 'bg-[#fff0ed] text-[#a94a3c]' : item.status === 'Em teste' ? 'bg-[#eaf2fb] text-[#3f72a2]' : 'bg-[#f0f3f1] text-[#7e8b84]'}`}>{item.status}</span><ChevronRight className="size-4 text-[#a6b0aa] transition group-hover:translate-x-0.5" /></span></button>; })}</div>
        </section>
      </div>
