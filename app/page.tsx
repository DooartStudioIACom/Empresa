...
function RoundsView() {
  ...
  async function createRound(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setRounds((current) => [payload.round!, ...current]); setSelectedRoundId(payload.round.id); setNewRoundOpen(false); setDraftItems([{ key: Date.now(), title: '', path: '', description: '' }]); form.reset(); setNotice({ tone: 'success', message: 'Rodada criada e liberada para a equipe.' });
  ...
  return <div>
    ...
    {loading ? <div className="mt-10 flex items-center justify-center gap-2 text-xs text-[#718078]"><LoaderCircle className="size-4 animate-spin" /> Carregando rodadas...</div> : !selectedRound ? <section className="mt-6 rounded-2xl border border-dashed border-[#cfdcd4] bg-white px-6 py-14 text-center"><ListChecks className="mx-auto size-7 text-[#668274]" /><h3 className="mt-4 text-sm font-semibold">Nenhuma rodada cadastrada</h3><p className="mt-2 text-xs text-[#78867e]">Crie a primeira rodada com os itens que chegaram da gerência.</p><Button type="button" onClick={() => setNewRoundOpen(true)} className="mt-5 bg-[#173e2c] text-white"><Plus /> Criar rodada</Button></section> : <div className="mt-6 grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
      <section className="overflow-hidden rounded-2xl border border-[#dce5df] bg-white">
        <div className="relative min-h-[310px] overflow-hidden border-b border-[#274a38] bg-[radial-gradient(circle_at_82%_48%,#2b5b40_0%,#173e2c_32%,#102d20_68%)] p-6 text-white sm:p-8"><div className={`absolute right-[7%] top-1/2 size-[270px] -translate-y-1/2 rounded-full transition-all duration-1000 ${beaconPowered ? 'beacon-aura opacity-100' : 'bg-white/[.025] opacity-50'}`} /><div className="absolute right-[4%] top-1/2 size-[330px] -translate-y-1/2 rounded-full border border-white/[.06]" /><div className="absolute right-[1%] top-1/2 size-[390px] -translate-y-1/2 rounded-full border border-white/[.035]" /><div className="relative z-[2] max-w-full sm:max-w-[62%]"><div className="flex flex-wrap items-center gap-2"><Badge className={beaconPowered ? 'bg-[#d7ff66] text-[#173e2c]' : 'bg-white/12 text-white'}>{selectedRound.status.toUpperCase()}</Badge><span className="inline-flex items-center gap-1.5 text-[10px] font-semibold uppercase tracking-[.12em] text-white/55"><span className={`size-1.5 rounded-full ${beaconPowered ? 'animate-pulse bg-[#d7ff66]' : 'bg-white/35'}`} />{beaconPowered ? 'Farol de qualidade ativo' : 'Aguardando primeiro teste'}</span></div><h2 className="mt-4 text-2xl font-semibold tracking-[-.035em]">{selectedRound.title}</h2><p className="mt-1.5 text-xs text-white/55">{selectedRound.version} · prazo {formatRoundDate(selectedRound.deadline)}</p>{selectedRound.description && <p className="mt-4 max-w-xl text-xs leading-5 text-white/68">{selectedRound.description}</p>}<div className="mt-7 max-w-xl"><div className="flex items-end justify-between"><div><p className="text-[10px] font-semibold uppercase tracking-[.12em] text-white/45">Energia da rodada</p><p className="mt-1 text-sm font-semibold">{tested} de {selectedRound.items.length} testes concluídos</p></div><span className="text-3xl font-semibold tracking-[-.06em] text-[#d7ff66]">{progress}%</span></div><div className="mt-3 h-2.5 overflow-hidden rounded-full bg-white/10 ring-1 ring-white/5"><div style={{ width: `${progress}%` }} className="h-full rounded-full bg-[linear-gradient(90deg,#68b27f,#d7ff66)] shadow-[0_0_16px_#d7ff6690] transition-all duration-700" /></div></div></div><div className={`pointer-events-none absolute right-[5%] top-1/2 z-[1] hidden w-[300px] -translate-y-1/2 transition-all duration-1000 sm:block ${beaconPowered ? 'beacon-float opacity-100 drop-shadow-[0_0_28px_#d7ff6655]' : 'opacity-70 grayscale-[.28]'}`}><img src="/rounds/quality-beacon-off.png" alt={beaconPowered ? 'Farol de qualidade aceso' : 'Farol de qualidade apagado'} className={`h-auto w-full transition-all duration-1000 ${beaconPowered ? 'brightness-110 saturate-[1.35] contrast-110' : 'brightness-75 saturate-50'}`} />{beaconPowered && <div className="beacon-core absolute left-1/2 top-[36%] size-[112px] -translate-x-1/2 -translate-y-1/2 rounded-full bg-[#d7ff66]/35 blur-2xl" />}</div></div>
        <div className="grid grid-cols-2 gap-px border-b border-[#e5ebe7] bg-[#e5ebe7] sm:grid-cols-4">{[[pendingCount,'Pendentes','#7b8981'],[testingCount,'Em teste','#3970a6'],[approvedCount,'Aprovados','#347951'],[bugCount,'Com bug','#b64738']].map(([value,label,color]) => <div key={String(label)} className="bg-white px-4 py-3"><p style={{ color: String(color) }} className="text-lg font-semibold tracking-[-.04em]">{value}</p><p className="mt-0.5 text-[10px] text-[#7b8981]">{label}</p></div>)}</div>
