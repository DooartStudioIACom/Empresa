...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  const row = await env.DB.prepare('SELECT a.object_key,a.file_name,a.content_type,a.kind,r.id AS report_id,r.status,r.author_id,r.author_email FROM attachments a JOIN reports r ON r.id=a.report_id WHERE a.id=?').bind(id).first<{ object_key: string; file_name: string; content_type: string; kind: string; report_id: string; status: string; author_id: string; author_email: string }>();
  ...
  const teamVisibleImage = ['inline', 'screenshot'].includes(row.kind) && row.content_type.startsWith('image/');
  if (!isOwner && !shared && !roleAccess && !teamVisibleImage) return new Response('Anexo não disponível para o seu perfil', { status: 403 });
