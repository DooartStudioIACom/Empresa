...
export const attachments = sqliteTable('attachments', {
  id: text('id').primaryKey(), reportId: text('report_id').notNull().references(() => reports.id, { onDelete: 'cascade' }), objectKey: text('object_key').notNull().unique(), fileName: text('file_name').notNull(), contentType: text('content_type').notNull(), byteSize: integer('byte_size').notNull(), kind: text('kind').notNull(), githubShareToken: text('github_share_token').unique(), createdAt: integer('created_at').notNull(),
...
export const testRounds = sqliteTable('test_rounds', {
  id: text('id').primaryKey(), title: text('title').notNull(), version: text('version').notNull(), deadline: text('deadline').notNull(), description: text('description'), tags: text('tags').notNull().default('[]'), status: text('status').notNull().default('Em andamento'), authorId: text('author_id').notNull(), authorEmail: text('author_email').notNull(), createdAt: integer('created_at').notNull(), updatedAt: integer('updated_at').notNull(),
